...
export function LessonAdminEditButton({ lessonId }: LessonAdminEditButtonProps) {
  ...
  const loadEntryMutation = useMutation({
    mutationFn: async () => {
      return queryClient.fetchQuery(journalEntryByIdQueryOptions(lessonId));
    },
    onSuccess: (row) => {
      if (!row) {
  ...
  return (
    <>
      <Button
        ...
        onClick={() => loadEntryMutation.mutate()}
        disabled={loadEntryMutation.isPending}
        data-test="lesson-admin-edit-button"
      >
        {loadEntryMutation.isPending ? (
