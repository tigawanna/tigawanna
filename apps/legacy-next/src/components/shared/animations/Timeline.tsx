...
function TimelineItem({ year, title, description, index, isActive, onClick }: TimelineItemProps) {
  // Use our optimized animation hook with accessibility support
  const { ref, isInView, style } = useOptimizedAnimation("fadeInUp", {
    once: true,
    threshold: 0.5,
    delay: index * 0.1,
    duration: 0.6,
  ...
  return (
    ...
    >
      ...
      {/* Line and dot */}
      <div
        className={`absolute top-1/2 ${index % 2 === 0 ? "right-0 translate-x-1/2" : "left-0 -translate-x-1/2"} hidden md:block`}
      >
        <div
          className={`w-4 h-4 rounded-full ${isActive ? "bg-primary" : "bg-secondary"} transition-colors duration-300`}
        ></div>
      </div>
    </motion.div>
  );
