...
import type { ViewerLibraries } from "../libraries/helpers/api";
...
export function TopJavascriptFramewoorks({
  libs,
}: TopJavascriptFramewoorksProps) {
  if (!libs || !libs?.framework_stats) return null;
  const framework_stats = Object.entries(libs?.framework_stats).map(
    ([k, v]) => {
